...
def get_top_changes(df, column_name="price_change_percentage_h1"):
    ...
    return top_changes[
        ["name", column_name, "transactions_h1_sells", "transactions_h1_buys"]
    ]
...
def plot_price_change(df, log_scale=False):
    ...
    price_changes_24h = [
        float(change) if change else 0 for change in df["price_change_percentage_h1"]
    ]
    fig, ax = plt.subplots()
    sns.barplot(x="price_change_percentage_h1", y="name", data=df, ax=ax)
    ax.set(
        xlabel="Price Change Percentage (24h)",
        title="Price Change Percentage (24h) by Pool",
    )
    if log_scale:
        ax.set_xscale("log")
